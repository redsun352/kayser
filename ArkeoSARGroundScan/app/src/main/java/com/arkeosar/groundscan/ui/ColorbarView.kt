...
class ColorbarView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Gradient matches AnomalyColorScale's stops, drawn top(high) to
        // bottom(low) since that's how the reference colorbar reads.
        val colors = intArrayOf(
            colorToArgb(AnomalyColorScale.colorFor(1f)),
            colorToArgb(AnomalyColorScale.colorFor(0.8f)),
            colorToArgb(AnomalyColorScale.colorFor(0.45f)),
            colorToArgb(AnomalyColorScale.colorFor(0f))
        )
        val positions = floatArrayOf(0f, 0.2f, 0.55f, 1f)
